...
* `bit_price_ps` и `cell_price_ps`: Эти параметры представляют [цены на хранение](https://docs.ton.org/develop/howto/fees-low-level#storage_fees) одного бита или одной ячейки информации в основных воркчейнах блокчейна TON. 

* `mc_bit_price_ps` и `mc_cell_price_ps`: Эти параметры представляют цены на вычислительные ресурсы конкретно в мастерчейне TON.
...
* `freeze_due_limit` и `delete_due_limit`: Представляют ограничения, связанные с замораживанием и удалением контрактов, которые не могут оплатить хранение или вычисления.
...
В этих параметрах задаются ограничения на блок. Они нужны, чтобы злоумышленник-валидатор не мог нарушить работу сети, создав огромный блок.
...
* - `soft_limit`: Мягкое ограничение - это идеальный размер блока.
...
:::info
При недостатке нагрузки на шард и, соответсвенно, желании смержиться с соседним, `soft_limit` определяет состояние, сверх которого перестают обрабатываться внутренние (internal) сообщения, но продолжат внешние (external). В случае достижения `hard_limit` блок не может выполнять никакие действия по слиянию (и, вероятно, запросит разделение на шарды).
:::
