...
словарь параметров платы за хранение: с какого времени действует, сколько стоит хранение бит/ячеек, в соответствии с 
...
Этот параметр определяет параметры конфигурации для цен на хранение в сети мастер-цепочки блокчейна TON.

### Параметры конфигурации:

#### Настройка цен газа: 

* `gas_flat_pfx`: Похоже, это префикс для фиксированных цен на газ и лимитов.

* `flat_gas_limit`: 100: Это может представлять максимальное количество газа, которое может быть потреблено за транзакцию.

* `flat_gas_price`: 1000000: Это может представлять собой цену на газ в сети.
